...
> 🎉 **本版本（v3.0）起，蓝湖 MCP 默认输出统一的"完整需求文档 + 可交互 HTML 原型"，
...
| 需要用户额外决策 | 拿到 STAGE 2 结果即可直接进入 STAGE 4 |
| analysis_mode 参数必需 | analysis_mode 参数保留兼容，可忽略 |

**统一视角同时包含：**
- 🛠️ **开发视角**：字段规则表、功能清单、业务规则、流程图
- 🧪 **测试视角**：正向/异常场景、字段校验边界值、状态变化表、联调测试点
- 🎯 **评审视角**：模块概览、依赖关系、开发顺序建议、风险项

### STAGE 2 — 分组深度分析（统一视角）

- 逐组调用 `lanhu_get_ai_analyze_page_result(page_names=[该组], mode="full")`
- **无需指定 analysis_mode**（参数已废弃，传了也会被忽略）
...
- 汇总 STAGE 2 结果，校验完整性（字段、场景、依赖、风险四个维度）
- 统计变更类型（新增 X / 修改 Y / 未明确 Z）
- 生成"待确认清单"

### STAGE 4 — 生成交付文档（v3.0 统一格式）

...
3. 给出具体修复步骤（参考下方"标准引导模板"）。
4. **在用户确认已修复并重试之前，不要继续往下走流程**。

**标准引导模板（当看到 `config_missing`）：**

```
🔧 蓝湖 MCP 还未配置 LANHU_COOKIE，请先完成以下步骤：

1. 复制模板：cp "<example_path>" "<env_path>"
2. 编辑 .env，把 LANHU_COOKIE 改成你的真实 Cookie
   （替换 your_lanhu_cookie_here）
3. 重启 Claude Code / Cursor 让新配置生效
4. 完成后告诉我"配置好了"，我会重新调用 MCP 获取需求
```
...
> ⚠️ **收到页面列表后，立即用 TodoWrite 创建框架跟踪进度。**
> 仅获取页面列表（用户说"看看有哪些页"）时，STAGE 0 输出列表即可停止。
...
digraph four_stage {
    ...
    s0 [label="STAGE 0 展示页面列表\n(按模块分组，等待用户选择范围)", fillcolor="#d5e8d4"];
    s1 [label="STAGE 1 全局快速浏览\n(可选，仅用户要求时执行)\ntext_only, page_names=all", fillcolor="#fff2cc"];
    s2 [label="STAGE 2 按需深度分析\n(核心)\nfull, 用户选择的页面分组", fillcolor="#d5e8d4"];
    s3 [label="STAGE 3 汇总验证 + 生成交付文档\n(Markdown + 可交互HTML)", fillcolor="#dae8fc"];

    s0 -> s1 [label="用户要求先整体看看"];
    s0 -> s2 [label="用户选择分析范围"];
    s1 -> s2;
    s2 -> s3;
}
```

### STAGE 0 — 展示页面列表，等待用户选择范围

- 按模块分组展示页面，**不下载任何页面内容**
- 询问用户：分析全部 / 指定模块 / 指定页面
- ⛔ **【禁止】默认调用 `page_names="all"`**，必须等用户确认范围后再进入 STAGE 2

### STAGE 1（可选）— 全局快速浏览

- **仅当用户明确要求"先整体看看"、"快速扫一遍"才执行**
...
- 本阶段不要求用户选择分析模式，统一走"完整需求文档 + 可交互HTML"

### STAGE 2 — 按需深度分析（核心）

- 根据用户选择的模块/页面分组
- 调用：`lanhu_get_ai_analyze_page_result(page_names=[选中的页面], mode="full")`
- **无需指定 `analysis_mode`**（参数已废弃，传了也会被忽略），默认统一为"完整需求文档"输出
- 一次输出覆盖：字段规则 + 测试场景 + 模块依赖 + 评审要点
- 用 TodoWrite 把分组拆成每模块一项
- **每个分组必须做变更类型识别**：🆕新增 / 🔄修改 / ❓未明确 + 判断依据

**每组分析输出必须包含以下结构（统一完整视角 v3.0）：**
...
- 合并 STAGE 2 所有分组结果，校验完整性（字段、场景、依赖、风险四个维度）
...
| 需要用户额外决策 | STAGE 2 结果直接进入 STAGE 3 |
...
- 多个页面分析时，每个页面独立创建目录
- HTML文件**必须**包含：页面截图、设计样式参考、颜色值、字体规格、字段规则表格、可折叠测试场景、模块导航、暗色/亮色切换
- MD文件**必须**包含：完整的分析结果（功能清单、字段规则、测试场景、流程图、待确认事项）

...
digraph workflow {
    ...
    s1 [label="STAGE 1 按需深度分析\n(核心)\nmode=full, 用户选择的页面分组", fillcolor="#d5e8d4"];
    s2 [label="STAGE 2 汇总验证 + 生成交付文档\n(Markdown + 可交互HTML)", fillcolor="#dae8fc"];

    s0 -> s1 [label="用户选择分析范围"];
    s1 -> s2;
}
```
...
- 按模块分组展示页面，**不下载任何页面内容**
- 询问用户：指定模块 / 指定页面
- ⛔ **【禁止】提供"分析全部"选项**，⛔ **【禁止】默认调用 `page_names="all"`**，⛔ **【禁止】调用 `mode="text_only"`**，必须等用户明确选择具体页面/模块后再进入 STAGE 1
- 如果用户说"全部"、"都看看"、"整体"等，仍需按模块列出让用户逐个确认选择范围，不能一次性拉全部
...
- 合并 STAGE 1 所有分组结果，校验完整性（字段、场景、依赖、风险四个维度）

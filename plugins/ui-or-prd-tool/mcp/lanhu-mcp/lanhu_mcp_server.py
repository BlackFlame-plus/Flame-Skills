...
class LanhuExtractor:
    ...
    async def download_resources(self, url: str, output_dir: str, force_update: bool = False,
                                 target_pages: list = None, text_only: bool = False) -> dict:
        """
        下载Axure资源（支持智能缓存+按需下载）

        Args:
            url: 蓝湖文档URL
            output_dir: 输出目录
            force_update: 强制更新，忽略缓存
            target_pages: 目标页面文件名列表（如['jc云-俱乐部.html']），None=下载全部
            text_only: 仅文本模式，跳过CSS/图片下载（大幅提速）
        ...
        # ⚡ 按需下载：过滤目标页面
        pages_to_download = pages.items()
        if target_pages:
            # target_pages 是不带.html的文件名，需要匹配
            target_set = set(target_pages)
            pages_to_download = [
                (html_filename, page_info)
                for html_filename, page_info in pages.items()
                if html_filename.replace('.html', '') in target_set
            ]

        for html_filename, page_info in pages_to_download:
            ...
            if page_mapping_md5:
                ...
                # 下载所有依赖资源
                # ⚡ text_only模式：跳过CSS和图片，只保留必需JS（document.js）
                if text_only:
                    # 极简模式：只下载JS（保证渲染能运行），跳过CSS/图片
                    await self._download_page_resources(
                        page_mapping, output_path, skip_document_js=(not is_first_page),
                        skip_styles=True, skip_images=True
                    )
                else:
                    await self._download_page_resources(
                        page_mapping, output_path, skip_document_js=(not is_first_page)
        ...
        cache_meta = {
            ...
            'downloaded_pages': downloaded_files,
    ...
    async def _download_page_resources(self, page_mapping: dict, output_dir: Path, skip_document_js: bool = False,
                                       skip_styles: bool = False, skip_images: bool = False):
        """
        下载页面资源（支持按需跳过，大幅提速）

        Args:
            skip_styles: 跳过CSS下载（text_only模式用，速度+80%）
            skip_images: 跳过图片下载（text_only模式用，速度+100%+）
        """
        tasks = []
...
async def lanhu_get_pages(
    ...
    try:
        ...
        # ✅ AI行为指令模块化（2026.06.25 优化：Token 减少 80%）
        from ai_persona import AIPersona
        result['__AI_INSTRUCTION__'] = AIPersona.get_workflow(mode_options)
...
async def lanhu_get_ai_analyze_page_result(
    ...
    try:
        ...
        # ⚡ 按需下载资源（支持智能缓存+目标页面过滤+text_only极速模式）
        is_text_only = (mode == "text_only")
        download_result = await extractor.download_resources(
            url, resource_dir,
            target_pages=target_pages,  # 只下载目标页面资源
            text_only=is_text_only      # text_only跳过CSS/图片
        )
        ...
        results = await screenshot_page_internal(resource_dir, target_pages, screenshot_dir, return_base64=False, version_id=version_id)
    ...
    try:
        ...
        for design in target_designs:
            ...
            try:
                ...
                img_filepath = design_output_dir / img_filename
            ...
            try:
                ...
                html_filepath = design_output_dir / html_filename
        ...
        if not keep_raw_data:
            ...
            try:
                if design_output_dir.exists():
                    shutil.rmtree(design_output_dir)
            except Exception:
                pass  # 清理失败不影响结果返回
